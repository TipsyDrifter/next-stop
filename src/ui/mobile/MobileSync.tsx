 ...
 * 手機沒有備份三件套 ⇒ 沒有還原（規則②是桌機的事），其餘與桌機同一套字。
      ...
      <section className="m2-sec" aria-labelledby="m2-sec-sync">
            ...
            <div className="m2-sy-actions">
              <button
                ...
                onClick={() =>
                  askConfirm({
                    title: "要改用那份嗎？",
                    body: "這台現有的車票與記錄會被那份取代；還沒送出的修改另存成檔、不會自動併回。",
                    confirmLabel: "改用那份",
                    danger: true,
        ...
        {configured && updatingCreds ? (
          ...
          <>
            ...
            <PassphraseBlock status={status} working={working} error={formError} onSubmit={changePassphrase} />
...
function ChoiceBlock({
  ...
  return (
    <div className="m2-block m2-sy-epoch">
      ...
      <p className="m2-note">
        雲端上已有一份（{report.remote_devices} 台裝置在用），這台也有 {report.local_alive} 張活著的票。要怎麼做？
      </p>
      {error && (
        <p className="m2-note is-fail" role="alert">
          {error}
      ...
      <p className="m2-note">這台現有的會先匯出成一份 JSON 到「下載／NextStop」，再換成雲端那份。</p>
      <div className="m2-sy-actions">
        <button type="button" className="ns-btn btn-ghost m2-sy-go" disabled={working} onClick={onCancel}>
          取消
        </button>
...
function PassphraseBlock({
  ...
  const ready = next1.length >= 8 && next1 === next2 && !working;
  ...
  return (
    <div className="m2-block">
      <span className="m2-block-label">密語</span>
      <p className="m2-note">改密語只重包雲端上的鑰匙，資料不重傳；其他已加入的裝置不受影響，之後新加入的裝置要用新密語。</p>
      {keySealed === false && <p className="m2-note">升級後第一次：這次會把鑰匙封存到雲端（新密語可以與現在相同）。</p>}
      <p className="m2-note">忘了現在的密語也沒關係——這台的鑰匙還在，現在的密語可以留白，直接設一個新的就好。</p>
      <input className="m2-input" type="password" value={current} autoComplete="current-password" placeholder="現在的密語（可留白）" onChange={(e) => setCurrent(e.target.value)} aria-label="現在的密語（可留白）" />
